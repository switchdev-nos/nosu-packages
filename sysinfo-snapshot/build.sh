...
if [ -n "$PKG_SYSINFO_SNAPSHOT_GIT" ] && [ ! -d "$SYSINFO_SNAPSHOT_BUILDDIR" ]; then
  if [ -n "$PKG_SYSINFO_SNAPSHOT_GIT_BRANCH" ]; then
    CLONE="$PKG_SYSINFO_SNAPSHOT_GIT -b $PKG_SYSINFO_SNAPSHOT_GIT_BRANCH $SYSINFO_SNAPSHOT_BUILDDIR"
  else
    CLONE="$PKG_SYSINFO_SNAPSHOT_GIT $SYSINFO_SNAPSHOT_BUILDDIR"
  fi
  echo "Clone: $CLONE"
  git clone $CLONE
